...
public class FaqTest {
    ...
    public FaqTest(int index, String question, String answer) {
        this.index = index;
    ...
    public static Object[][] result() {
        return new Object[][]{
                ...
                {2, "Как рассчитывается время аренды?", "Допустим, вы оформляете заказ на 8 мая. Мы привозим самокат 8 " +
                        "мая в течение дня. Отсчёт времени" +
                ...
                {3, "Можно ли заказать самокат прямо на сегодня?", "Только начиная с завтрашнего дня. Но скоро станем" +
                        " расторопнее."},
                {4, "Можно ли продлить заказ или вернуть самокат раньше?", "Пока что нет! Но если что-то срочное" +
                        " — всегда можно позвонить в поддержку по красивому номеру" +
                        " 1010."},
                {5, "Вы привозите зарядку вместе с самокатом?", "Самокат приезжает к вам с полной зарядкой. Этого" +
    ...
    public void faqListTest() {
        ...
        objMainPage.scrollToDropdownList(); // прокручиваем страницу до нужного элемента
        objMainPage.clickQuestion(question); // нажимаем на вопрос
        String actualAnswer = objMainPage.getAnswerText(index); // получаем текст ответа
        assertEquals(answer, actualAnswer); // сравниваем ответы
